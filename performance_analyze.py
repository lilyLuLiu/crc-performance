...
# --- Create client ---
client = OpenSearch(
    "https://osv3.app.intlab.redhat.com",
...
def get_cpu_data(item):#item list["cpu-Start","cpu-Stop"]
    CPU_data={}
...
def draw_by_platform(item, image_folder, data=Platform_DATA):
...
def draw_all():
    draw_by_platform("time-start",RESULT_FOLDER)
    draw_by_platform("time-stop",RESULT_FOLDER)
    draw_by_platform("memory-start",RESULT_FOLDER)
    draw_by_platform("memory-stop",RESULT_FOLDER)
    draw_by_platform("memory-deployment",RESULT_FOLDER)
...
def sort_by_platform(item):
    return df.groupby('category')[item].mean().reset_index().to_dict(orient='records')

def time_by_platform():
    result={}
    result["start"]=sort_by_platform("time-start")
    result["stop"]=sort_by_platform("time-stop")
...
def memory_by_platform():
    result={}
    result["start"]=sort_by_platform("memory-start")
    result["deployment"]=sort_by_platform("memory-deployment")
    result["stop"]=sort_by_platform("memory-stop")
    return result
...
def export_cpu_csv():
    ...
    for item in list:
        cpudata = get_cpu_data(item)
        final_df = pd.concat(cpudata.values(), ignore_index=True)
        final_df.to_csv(f"{folder}/{item}_analyze_data.csv", index=False)

time_platform = time_by_platform()
start_time, stop_time = time_by_bundle()
timeData = {
...
memory_platform = memory_by_platform()
start, deploy, stop = memory_by_bundle()
memoryData = {
...
html_output = template.render(timeData=timeData, memoryData=memoryData)
